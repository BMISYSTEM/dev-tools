...
const Aside = () => {
  ...
  return (
    <>
        <section className={`border-r transition-all  ease-in-out duration-500 min-w-12 ${expandMenu ? ' md:w-52 sm:12' : ' w-12'} h-full  overflow-auto md:flex flex-col hidden gap-5  `}>
            {/* menu de lineas */}
        ...
        <section className={`border-r md:hidden transition-all ease-in-out duration-500 overflow-hidden min-w-12 ${expandMenu ? ' sm:h-screen' : ' h-10'}   md:flex flex-col  gap-5  `}>
            {/* menu de lineas */}
            <section className="w-full flex items-center justify-center p-2 ">
                <button onClick={()=>{setExpandMenu(!expandMenu)
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`w-6 h-6 text-slate-200 transform transition-transform duration-500 ${!expandMenu ? '' : 'rotate-180'}`}>
